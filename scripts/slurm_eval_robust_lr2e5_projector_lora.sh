#!/usr/bin/env bash
#SBATCH --job-name=qwen-eval-plora
...
checkpoint_root=models/checkpoints/robust-acoustic-lora-r8-lr2e-5-projector-lora-seed42
selection_root=outputs/checkpoint-selection/robust-lr2e-5-projector-lora
...
root = Path("outputs/checkpoint-selection/robust-lr2e-5-projector-lora")
candidates = []
for step in (200, 400, 600, 800, 889):
    ...
    candidates.append({
        "step": step,
        "checkpoint": f"models/checkpoints/robust-acoustic-lora-r8-lr2e-5-projector-lora-seed42/checkpoint-{step}",
        "summary": str(path),
        "wer": summary["wer"],
...
report = {
    ...
    "experiment": "robust-lr2e-5-projector-lora",
    "config": "configs/robust-lr2e-5.json",
...
test_output="outputs/final-evaluation/robust-lr2e-5-projector-lora-checkpoint-${best_step}-test-200x14"
mkdir -p "$test_output"
...
dev = json.loads(Path("outputs/checkpoint-selection/robust-lr2e-5-projector-lora/selection-summary.json").read_text())
...
report = {
    ...
    "config": "configs/robust-lr2e-5.json",
    "selected_step": step,
    "selected_checkpoint": f"models/checkpoints/robust-acoustic-lora-r8-lr2e-5-projector-lora-seed42/checkpoint-{step}",
    "dev_selection": dev,
...
Path("outputs/final-evaluation/robust-lr2e-5-projector-lora-selected.json").write_text(json.dumps(report, indent=2) + "\n")
