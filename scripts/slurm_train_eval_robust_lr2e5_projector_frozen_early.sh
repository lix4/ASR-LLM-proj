#!/usr/bin/env bash
#SBATCH --job-name=qwen-early-pfrozen
...
checkpoint_root=models/checkpoints/robust-acoustic-lora-r8-lr2e-5-projector-frozen-early-seed42
selection_root=outputs/checkpoint-selection/robust-lr2e-5-projector-frozen-early
...
root = Path("outputs/checkpoint-selection/robust-lr2e-5-projector-frozen-early")
checkpoint_root = "models/checkpoints/robust-acoustic-lora-r8-lr2e-5-projector-frozen-early-seed42"
candidates = []
for step in (50, 100, 150, 200):
    ...
    candidates.append({
        "step": step,
        "checkpoint": f"{checkpoint_root}/checkpoint-{step}",
        "summary": str(path),
        "wer": summary["wer"],
...
report = {
    ...
    "experiment": "robust-lr2e-5-projector-frozen-early",
    "config": "configs/robust-lr2e-5.json",
...
test_output="outputs/final-evaluation/robust-lr2e-5-projector-frozen-early-checkpoint-${best_step}-test-200x14"
mkdir -p "$test_output"
...
selection_path = Path("outputs/checkpoint-selection/robust-lr2e-5-projector-frozen-early/selection-summary.json")
...
Path("outputs/final-evaluation/robust-lr2e-5-projector-frozen-early-selected.json").write_text(
