#!/usr/bin/env bash
#SBATCH --job-name=qwen-eval-lr2e5
...
checkpoint_root=models/checkpoints/robust-acoustic-lora-r8-lr2e-5-seed42
selection_root=outputs/checkpoint-selection/robust-lr2e-5
...
root = Path("outputs/checkpoint-selection/robust-lr2e-5")
candidates = []
for step in (200, 400, 600, 800, 889):
    ...
    candidates.append({
        "step": step,
        "checkpoint": f"models/checkpoints/robust-acoustic-lora-r8-lr2e-5-seed42/checkpoint-{step}",
        "summary": str(path),
        "wer": summary["wer"],
...
test_output="outputs/final-evaluation/robust-lr2e-5-checkpoint-${best_step}-test-200x14"
mkdir -p "$test_output"
...
python - "$best_step" "$test_output" <<'PY'
import json
import sys
from pathlib import Path
...
dev = json.loads(Path("outputs/checkpoint-selection/robust-lr2e-5/selection-summary.json").read_text())
...
report = {
    "config": "configs/robust-lr2e-5.json",
    "selected_step": step,
    "selected_checkpoint": f"models/checkpoints/robust-acoustic-lora-r8-lr2e-5-seed42/checkpoint-{step}",
    "dev_selection": dev,
...
Path("outputs/final-evaluation/robust-lr2e-5-selected.json").write_text(json.dumps(report, indent=2) + "\n")
